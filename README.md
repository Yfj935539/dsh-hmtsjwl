...
| 3D 球形神经网络 | 球心 = 偏好/交流核心节点（永不自动衰减）；第一层球面 = 工作区目录节点（每个工作区文件夹一个，与核心**永久连接**）；外球面 = 衍生记忆（按所属工作区方向扩散）；拖拽旋转、滚轮缩放、双击重置 |
| 动态连接 | 两节点交流（检索共激活）自动建连/强化（Hebbian）；**3 天无交互传输自动断开**；偏好/交流 ↔ 工作区目录为永久连接，除非用户主动归档工作区（右键节点） |
| 对话前记忆注入 | 每次对话 turn 前自动把关键记忆（强度×0.6+新鲜度×0.4）+ 最新工作状态注入系统提示；`DSH_HIPPOCAMPUS_PROMPT=0` 可关闭 |
| 每小时轨迹喂养 | 自动提炼会话轨迹（信号句 + 高频词）写入「会话精华」分支；记忆页「喂养轨迹」按钮全量喂养整个对话历史 |
| 自动维护 | 记忆库上限 1GB 超限自动优化（先提炼精华、再删过期无效、VACUUM 压缩）；过期记忆（>30 天）检索自动降权、弱过期自动归档/删除；活跃节点上限 1000 超出自动淘汰最弱（工作状态/精华豁免） |
